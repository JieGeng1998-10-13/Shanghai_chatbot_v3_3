...
template3 = """
请按照步骤执行：
1. 首先明确问题的主体，行为，和问问题的意图
    问题主体、行为、意图：{understand}
2. 浏览问题，根据参考内容回答问题，要求回答是法律三段式论证：
    问题：{question}
    参考：{reference}
    回答格式：
        a) 阐述案件事实，通过1.中的意图去明确案件的主要争议点
        b) 引用正确的法律（来源于参考）
        c) 通过b)引用的法律，对案件事实进行判定，说明判定结果
        d) 给出当事人建议
...
template4 = """
请根据以下规则进行问题分解，返回问题分解的中文结果。
注意：只根据问题内容进行分解，不要其他参考。
问题：{question}
    规则：1. 确定问题的核心内容
            目标：明确问题涉及的对象、行为和背景，抓住问题的主旨。
...
chain_understand = prompt3 | model_json
...
def call_problem_example(state):
    ...
    # 问题理解
    understand_reference = chain_understand.invoke({"question": question})
    # 检索子问题
    ...
    # 回答
    final_answer = chain3.invoke({"question": question, "reference": reference_lst, "understand": understand_reference})
